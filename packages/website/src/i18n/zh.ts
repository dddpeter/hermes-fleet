export default {
  ...
  hero: {
    ...
    downloadDesktop: '开始使用',
    latestRelease: '最新版本',
    getInstallers: '获取安装包',
    docsTitle: '文档',
    docsLibrary: '资料库',
    install: 'npm install -g hermes-web-ui',
    previewAlt: 'Hermes Fleet 仪表板预览',
  },
  features: {
    title: 'Hermes Agent 的本地控制台',
    desc: 'Hermes Fleet 把聊天、运行时管理、自动化、文件、Coding Agent 和桌面分发整合到一个工作区。',
    streaming: {
      title: 'Agent 聊天',
  ...
  screenshots: {
    ...
    items: [
      {
        src: '/image.gif',
        alt: 'Hermes Fleet 产品流程演示',
        title: 'Hermes Fleet 工作流',
  ...
  install: {
    ...
    desktop: {
      ...
      prereq: '桌面版已内置 Hermes Fleet 运行时。',
      downloads: [
        {
  ...
  docs: {
    ...
    gettingStarted: {
      title: '快速开始',
      intro: 'Hermes Fleet 是一个自托管的 Web 仪表板，用于管理 AI 对话、平台通道、定时任务等。它封装了 Hermes Agent CLI 并提供美观的 Web 界面。',
      install: {
        title: '安装',
      ...
      firstRun: {
        title: '首次运行',
        content: '首次启动时，Hermes Fleet 会自动生成认证令牌、初始化本地数据、启动 Hermes agent bridge 并在浏览器中打开仪表板。',
      },
      login: {
        title: '登录',
    ...
    configuration: {
      title: '配置说明',
      intro: 'Hermes Fleet 可通过环境变量进行配置。',
      envVars: {
        title: '环境变量',
        rows: [
          ['PORT', '服务器监听端口（默认：8648）'],
          ['BIND_HOST', '服务器绑定地址（默认：0.0.0.0）。如需 IPv6，请显式设置为 ::。'],
          ['HERMES_WEB_UI_HOME', 'Hermes Fleet 数据目录，用于认证 token、登录凭据、日志、数据库和默认上传目录'],
          ...
          ['HERMES_WEB_UI_STOP_GATEWAYS_ON_SHUTDOWN', 'Hermes Fleet 关闭时是否同时停止托管的 gateway 进程'],
      ...
      gateway: {
        ...
        content: '聊天运行通过 Hermes agent bridge 处理。它随 Hermes Fleet 服务一起运行，并直接连接 Hermes Agent runtime。HERMES_AGENT_BRIDGE_ENDPOINT 控制 Node 与 bridge broker 的连接地址；HERMES_AGENT_BRIDGE_WORKER_TRANSPORT 控制 broker 与各 Profile worker 的连接方式。Web UI 关闭以及 CLI/应用内重启默认会停止 bridge broker；只有明确希望跨重启保留 bridge session 时，才设置 HERMES_AGENT_BRIDGE_STOP_ON_SHUTDOWN=0。前端切换 Hermes Profile 只影响后续请求上下文，不会重启 bridge 或清理其他正在运行的任务。',
      },
      profiles: {
        title: '配置文件',
    ...
    features: {
      title: '功能详解',
      intro: '探索 Hermes Fleet 的核心功能。',
      chat: {
        title: 'AI 聊天',
        content: '通过 Socket.IO /chat-run 实时流式聊天。支持多会话管理、Markdown 渲染与语法高亮、工具调用检查、按 Profile 上传、按路径下载，以及 Ctrl+K 搜索 Hermes Fleet 本地会话库。',
      },
      kanban: {
        title: '看板管理',
    ...
    hermesStudioManual: {
      title: 'Hermes Fleet 客户端完整操作手册',
      intro: '这份手册面向 Hermes Fleet 桌面客户端和 Web 界面的使用、配置、运维、验收与内部培训场景，覆盖当前客户端可见导航、设置、弹窗、状态页、关键按钮、常见流程和风险提示。',
      open: {
        title: '打开完整手册',
      ...
      scope: {
        title: '适用范围',
        rows: [
          ['适用版本', 'Hermes Fleet 0.6.12 客户端操作手册'],
    ...
    esp32Intro: {
      ...
      intro: '这页发布 Hermes ESP32-C3 桌面 AI 对话小方盒的官网介绍资料。设备侧负责小屏、麦克风、扬声器、实体按键和局域网连接；Hermes Fleet 负责模型、语音识别、语音合成和会话运行。',
      open: {
        title: '打开完整介绍页',
        ...
        links: [
          {
            ...
            description: '包含产品图、硬件亮点、配网教程、连接 Hermes Fleet 的说明、规格与常见问题。',
          },
        ],
      },
      overview: {
        title: '设备定位',
        content: '这是配合 Hermes Fleet 使用的桌面硬件入口，不是离线独立大模型设备。ESP32-C3 提供本地交互与状态反馈，AI 能力由电脑端或服务器端 Hermes Fleet 以及用户配置的模型、STT、TTS 服务提供。',
      },
      requirements: {
        title: '使用前提',
        rows: [
          ['网络', 'ESP32 设备和 Hermes Fleet 需要在同一局域网内通信。'],
          ['服务地址', '设备端应填写电脑或服务器的局域网 IP + 端口，不能填写 127.0.0.1。'],
          ['语音能力', '需要在 Hermes Fleet 中配置可用的语音识别、语音合成和模型服务。'],
          ['适合用户', '适合 Hermes Fleet 用户、ESP32/Arduino/PlatformIO 玩家、开源硬件改造和桌面 AI 交互演示场景。'],
        ],
      },
      maintenance: {
        title: '维护说明',
        content: '后续如固件、配网页、端口、设备外观或 Hermes Fleet 设备接入流程变化，请同步更新静态 HTML 资源和本页的适用说明。',
      },
    },
    platforms: {
      title: '平台接入',
    ...
    api: {
      title: 'API 参考',
      intro: 'Hermes Fleet 提供本地 BFF API，并通过 Socket.IO 端点进行聊天流式通信。',
      local: {
        title: '本地 BFF 端点',
