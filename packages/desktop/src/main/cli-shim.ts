...
const PATH_MARKER_START = '# >>> Hermes Fleet CLI shim >>>'
const PATH_MARKER_END = '# <<< Hermes Fleet CLI shim <<<'
...
export function createShimContent(
  ...
  if (platform === 'win32') {
    ...
    return [
      ...
      'echo Unknown Hermes Fleet command: %~1 1>&2',
      ...
      'call :resolveRuntime',
      'if not exist "%PYTHON%" (',
      '  echo Hermes Fleet Python runtime not found at "%PYTHON%" 1>&2',
      '  echo Open Hermes Fleet once to finish runtime setup, then retry hermes-studio cli. 1>&2',
      '  exit /b 127',
      ')',
      'if not exist "%NODE%" (',
      '  echo Hermes Fleet Node runtime not found at "%NODE%" 1>&2',
      '  echo Open Hermes Fleet once to finish runtime setup, then retry hermes-studio cli. 1>&2',
      '  exit /b 127',
      ')',
      `"%NODE%" -e "${cliForwarder}" %*`,
      ...
      'if not exist "%NODE%" (',
      '  echo Hermes Fleet Node runtime not found at "%NODE%" 1>&2',
      '  echo Open Hermes Fleet once to finish runtime setup, then retry hermes-studio web. 1>&2',
      '  exit /b 127',
      ')',
      ...
      'echo.',
      'echo Commands:',
      'echo   ^(no command^)       Open Hermes Fleet desktop app',
  ...
  return [
    ...
    '',
    'Commands:',
    '  (no command)       Open Hermes Fleet desktop app',
    ...
    '}',
    'if [ ! -x "$APP" ]; then',
    '  echo "Hermes Fleet executable not found at $APP" >&2',
    '  exit 127',
    'fi',
    ...
    '    shift',
    '    if [ ! -x "$NODE" ]; then',
    '      echo "Hermes Fleet Node runtime not found at $NODE" >&2',
    '      echo "Open Hermes Fleet once to finish runtime setup, then retry hermes-studio web." >&2',
    '      exit 127',
    '    fi',
    ...
    '    ;;',
    '  *)',
    '    echo "Unknown Hermes Fleet command: $1" >&2',
...
export function createMcpShimContent(
  ...
  if (platform === 'win32') {
    return [
      ...
      'if not exist "%NODE%" (',
      '  echo Hermes Fleet Node runtime not found at "%NODE%" 1>&2',
      '  echo Open Hermes Fleet once to finish runtime setup, then retry hermes-studio-mcp. 1>&2',
      '  exit /b 127',
      ')',
      'if not exist "%SCRIPT%" (',
      '  echo Hermes Fleet MCP script not found at "%SCRIPT%" 1>&2',
      '  exit /b 127',
      ')',
  ...
  return [
    ...
    'if [ ! -x "$NODE" ]; then',
    '  echo "Hermes Fleet Node runtime not found at $NODE" >&2',
    '  echo "Open Hermes Fleet once to finish runtime setup, then retry hermes-studio-mcp." >&2',
    '  exit 127',
    'fi',
    'if [ ! -f "$SCRIPT" ]; then',
    '  echo "Hermes Fleet MCP script not found at $SCRIPT" >&2',
    '  exit 127',
    'fi',
...
export async function installHermesStudioCliShim(options: CliShimInstallOptions = {}): Promise<CliShimInstallResult> {
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing hermes-studio shim is not managed by Hermes Fleet' : undefined,
  }
}
...
export async function installHermesStudioMcpShim(options: McpShimInstallOptions = {}): Promise<CliShimInstallResult> {
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing hermes-studio-mcp shim is not managed by Hermes Fleet' : undefined,
  }
}
